...
export class CloudflareAGUIAdapter implements AGUIProtocol {
      ...
      if (chunk.tool_calls) {
        for (const toolCall of chunk.tool_calls) {
          if (!toolCallsInProgress.has(toolCall.id)) {
            // New tool call
            yield CloudflareAGUIEvents.toolCallStart(
              runId,
              toolCall.id,
  ...
  getCapabilities() {
    return this.client.getModelCapabilities(
      this.options.model || '@cf/meta/llama-3.1-8b-instruct'
    );
  }
